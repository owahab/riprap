...
Delete that line once configured; `bin/verify-stubs` reports what is outstanding.
...
- ✅ `src/models/user.ts`, `bin/hooks/lib/secret-patterns.sh`
...
skeleton to copy. `.claude/instructions/guardrail-template.md` is the doc shape.

Run `bin/verify-hook-wiring` after adding one — a hook that exists but is not wired is
