...
| | `bin/hooks/git/` | `bin/hooks/claude/` |
|---|---|---|
...
```
bin/hooks/lib/secret-patterns.sh   ← the definition of "this looks like a credential"
    ├── bin/hooks/git/pre-commit         enforces it at commit time
    └── bin/hooks/claude/lint-secrets.sh enforces it at tool-call time
```
...
`bin/setup` does it, and is safe to re-run:

```bash
git config core.hooksPath bin/hooks/git
```

That is the whole installation: the hooks are version-controlled files, and git is pointed at the
directory holding them.

**Why this beats symlinking into `.git/hooks`:**
...
- **Survives a fresh clone.** `.git/hooks` is local state — it is not cloned, so a new checkout has no
  hooks at all until somebody remembers to re-run an install step. `core.hooksPath` is one command in
  `bin/setup`, which people already run.
