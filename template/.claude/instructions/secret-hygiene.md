...
2. **A PreToolUse hook** — `bin/hooks/claude/lint-secrets.sh` blocks reads, blocks Bash
...
Layers 2 and 3 share one pattern library, `bin/hooks/lib/secret-patterns.sh`, so a rule
