---
name: branch-cleaner
description: Prune merged and stale Git branches and triage pull requests that have gone quiet, keeping the repository tidy. Use when the user runs /branch-cleaner or asks to prune, delete, clean up, or tidy old branches or stale PRs.
license: MIT
---
...
```bash
/branch-cleaner --merged-only    # Only branches merged into the base branch
/branch-cleaner --stale-only     # Only branches past the staleness threshold
/branch-cleaner --orphans-only   # Only branches whose upstream is gone
/branch-cleaner --branches-only  # Skip the pull request steps
```
